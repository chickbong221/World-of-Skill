...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    # MoSS: task ids must NOT reach the encoder/decoder, otherwise the model
    # reads the task label and subset discovery is vacuous.
  ...
  def loss(self, carry, obs, prevact, dataact, training):
    ...
    if self.ismoss:
      moss_con = f32(~obs['is_terminal'])
      dyn_carry, dyn_entries, los, repfeat, mets = self.dyn.loss(
          dyn_carry, tokens, prevact, reset, training,
          task_id=obs['task_id'], reward=obs['reward'], cont=moss_con)
    else:
      dyn_carry, dyn_entries, los, repfeat, mets = self.dyn.loss(
          dyn_carry, tokens, prevact, reset, training)
    losses.update(los)
    ...
    if self.config.get('offline', {}).get('enabled', False):
      behavior = self.pol(self.feat2tensor(repfeat), 2)
      bc_logp = sum([behavior[k].logp(sg(dataact[k])) for k in dataact])
      losses['bc_policy'] = f32(~obs['is_last']) * -bc_logp
      metrics['bc/logp'] = bc_logp.mean()
  ...
  def report(self, carry, data):
    ...
    _, (new_carry, entries, outs, mets) = self.loss(
        carry, obs, prevact, dataact, training=False)
    metrics.update(mets)
    ...
    # Video preds
    def to_report_rgb(x):
      # Some image observations flatten multiple RGB cameras into the channel
      # dimension. The report video path expects displayable RGB, so tile
      # camera views horizontally while leaving model tensors unchanged.
      channels = x.shape[-1]
      if channels == 3:
        return x
      if channels % 3 == 0:
  ...
  def _apply_replay_context(self, carry, data):
    ...
    dataact = {k: data[k] for k in self.act_space}
    prepend = lambda x, y: jnp.concatenate([x[:, None], y[:, :-1]], 1)
    prevact = {k: prepend(prevact[k], data[k]) for k in self.act_space}
    if not self.config.replay_context:
      return carry, obs, prevact, dataact, stepid
    ...
    rep_dataact = {k: rhs(data[k]) for k in self.act_space}
    rep_stepid = rhs(stepid)

    first_chunk = (data['consec'][:, 0] == 0)
    carry, obs, prevact, dataact, stepid = jax.tree.map(
        lambda normal, replay: nn.where(first_chunk, replay, normal),
        (carry, rhs(obs), rhs(prevact), rhs(dataact), rhs(stepid)),
        (rep_carry, rep_obs, rep_prevact, rep_dataact, rep_stepid))
    return carry, obs, prevact, dataact, stepid
